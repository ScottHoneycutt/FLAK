...
public class EnemySAMTurretTargeting : MonoBehaviour
{
    //Public variables----
                ...
                if (!hitRegister)
                {
                    ...
                    if ((target.position - turret.position).magnitude <= maxRange && (target.position - turret.position).magnitude >= minRange)
                    {
                        ...
                        if (fireTimer >= weaponPeriod)
                        {
                            ...
                            if (level1Turret)
                            {
                                missile = ObjectPoolManager.manager.SpawnFromPool("Enemy SAMs", turretPosition, turretRotation);
                            }
                            else
                            {
                                missile = ObjectPoolManager.manager.SpawnFromPool("Enemy SAMs Level 2", turretPosition, turretRotation);
                            }
