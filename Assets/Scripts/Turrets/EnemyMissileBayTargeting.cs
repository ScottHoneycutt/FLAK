...
public class EnemyMissileBayTargeting : MonoBehaviour
{
    //Public variables----
        ...
        if (target != null)
        {
            ...
            if ((target.transform.position - turret.transform.position).magnitude <= maxRange && (target.transform.position - turret.transform.position).magnitude >= minRange)
            {
                ...
                if (fireTimer >= weaponPeriod)
                {
                    ...
                    {
                        missile = ObjectPoolManager.manager.SpawnFromPool("Enemy Missile Bay Rockets", turretPosition, turretRotation);
