...
public class AlliedShipMovement : MonoBehaviour
{
    //Public variables----
    ...
    private List<GameObject> enemyBoats = new List<GameObject>();
    ...
    void Update()
    {
        ...
        if (advance)
        {
            ...
            if (timer >= refreshPeriod)
            {
                ...
                enemyBoats = Identifiers.identifier.ReturnEnemyShipTargeters();

                //Enemy boats use approachDistance----
                foreach (GameObject targeter in enemyBoats)
                {
                ...
                if (canMove)
                {
                    ...
                    if (movesLeft)
                    {
                        bool hitRegister = Physics.Raycast(rightFront.transform.position, Vector3.left, out hit, terrainDistance, mask);
