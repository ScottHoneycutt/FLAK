...
public class AlliedPhalanxTargeting : MonoBehaviour
{
    //Public variables----
    ...
    void FixedUpdate()
    {
        ...
        if (detectionTimer > detectionRefreshDelay)
        {
            ...
            nonMissileTargets = Identifiers.identifier.ReturnAlliedFlakMissileTargets();
            missileTargets = Identifiers.identifier.ReturnEnemyMissiles();
        ...
        if (target != null)
        {
            ...
            bool hitRegister = Physics.Raycast(turret.transform.position, rayDirection, out hit, range, mask);
