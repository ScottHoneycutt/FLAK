...
public class AlliedFlakTargeting : MonoBehaviour {
    ...
    public Object flakExplosionPrefab;
            ...
            if (!hitRegister)
            {
                ...
                if ((target.position - turret.position).magnitude <= maxRange && (target.position - turret.position).magnitude >= minRange)
                {
                    ...
                    if (fireTimer >= weaponPeriod)
                    {
                        ...
                        ObjectPoolManager.manager.SpawnFromPool("Allied Flak Explosions", flakPoint, rotation);
                    }
                }
            }
        }
