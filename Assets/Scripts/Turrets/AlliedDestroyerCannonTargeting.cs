...
public class AlliedDestroyerCannonTargeting : MonoBehaviour
{
    //Public variables----
    ...
    void FixedUpdate()
    {
        ...
        if (detectionTimer > detectionRefreshDelay)
        {
            ...
            List<GameObject> possibleTargets = Identifiers.identifier.ReturnEnemyShipTargeters();
        ...
        if (target != null)
        {
            ...
            if ((target.transform.position - barrels.transform.position).magnitude <= maxRange && (target.transform.position - barrels.transform.position).magnitude >= minRange)
            {
                ...
                if (fireTimer >= weaponPeriod)
                {
                    ...
                    firingSound.Play();
                    ObjectPoolManager.manager.SpawnFromPool("Allied Shell Points", shellPoint, rotation);
                }
            }
        }
