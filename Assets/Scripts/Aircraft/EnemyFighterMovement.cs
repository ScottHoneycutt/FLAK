...
public class EnemyFighterMovement : MonoBehaviour {
    ...
    private float takeoffTargetAngle = 180;
    ...
    public float empGravitySpeed = 60;
    public float empAirResistance = 20;
    public float empMaxFallSpeed = 150;
    //Private EMP variabbles----
    ...
    void FixedUpdate()
    {
        ...
        if (!healthStat.isEmped)
        {
            ...
            List<GameObject> missiles = Identifiers.identifier.ReturnAlliedMissiles();
            //Setup for evasion----
            ...
            List<GameObject> hostiles = Identifiers.identifier.ReturnEnemyFlakMissileTargets();
            //Setup for chase----
            bool engaged = false;
            target = null;
            ...
            else if (engaged)
            {
                ...
                if (Vector3.Distance(target.transform.position, patrolPoint1.transform.position) < chasingDistance || Vector3.Distance(target.transform.position, patrolPoint2.transform.position) < chasingDistance || Vector3.Distance(target.transform.position, fighter.position) < chaseTriggerDistance / 2)
                {
                    ...
                    if (target.transform.position.y < fighter.position.y)
                    {
        ...
        //Falling out of the sky if EMPed----
        else
        {
            //Air resistance----
            if (velocityX > 10)
            {
                velocityX = velocityX - empAirResistance * Time.deltaTime;
            }
            ...
            //Gravity and falling----
            if (velocityY > -empMaxFallSpeed)
            {
                velocityY = velocityY - empGravitySpeed * Time.deltaTime;
            }
            //Slowing fall if it is greater than fallspeed----
            if (velocityY < -empMaxFallSpeed)
            {
                velocityY = velocityY + empAirResistance * Time.deltaTime;
            }
            //Moving aircraft by velocity vectors----
            transform.Translate(new Vector3(velocityX, velocityY, 0) * Time.deltaTime, Space.World);
        }
    }

    //Aircraft can crash if and only if it is EMPed----
