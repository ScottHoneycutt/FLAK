...
public class EnemyFighterFireControl : MonoBehaviour {
    //Public variables----
        ...
        if (!healthStat.isEmped)
        {
            ...
            if (firing)
            {
                ...
                if (missileReloadTimer >= missileReloadTime.GetFloat())
                {
                    ...
                    if (!level2)
                    {
                        ObjectPoolManager.manager.SpawnFromPool("Enemy Air Missiles", fighter.position, rotation);
                    }
                    else
                    {
                        ObjectPoolManager.manager.SpawnFromPool("Enemy Air Missiles Level 2", fighter.position, rotation);
                    }
